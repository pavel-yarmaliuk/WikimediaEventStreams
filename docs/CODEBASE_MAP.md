---
last_mapped: 2026-05-05T13:07:43Z
total_files: 15
total_tokens: 4504
---
...
> Auto-generated by Cartographer. Last mapped: 2026-05-05T13:07:43Z
...
**Purpose**: Connects to the Wikimedia SSE API and yields parsed JSON edit events as an infinite, auto-reconnecting iterator.

**Key exports**:
- `read_stream(url) -> Iterator[dict]` — infinite generator, reconnects on any `RequestException`
- `parse_event(line) -> dict | None` — strips `data:` prefix and JSON-parses SSE lines

**Dependencies**: `requests`

**Patterns**: Generator/lazy streaming; manual SSE parsing; outer `while True` reconnect loop.

**Gotchas**:
- No backoff on reconnect — a dead stream will hammer the endpoint immediately.
...
- `main()` — infinite publish loop (no bounded run mode)

**Dependencies**: `stream_reader`, `kafka-python-ng`
...
**Key exports**:
- `main(limit=None)` — main loop with optional event count limit (useful for testing)

**Dependencies**: `stream_reader`, `neo4j_writer`, `python-dotenv`

**Patterns**: `try/finally` ensures `writer.close()`. CLI `--limit` flag for bounded runs.
...
- All Python modules use `logging` (not `print`) for operational output.
- Neo4j writes use `MERGE` for idempotency — safe to replay events.
- `stream_reader.py` is the single SSE abstraction; both entry points import it.
...
1. **No reconnect backoff** in `stream_reader.py` — broken SSE will hammer the endpoint.
2. **`REVERTED` edges are not true reverts** — they are created for any edit that has a prior revision.
3. **No Neo4j indexes/constraints** defined in code — production use requires explicit `CREATE CONSTRAINT` statements.
4. **Spark checkpoint is ephemeral** — `/tmp/wikimedia-checkpoint` is lost on container restart; stream resets to `latest`, skipping events.
5. **Kafka JAR fetched at runtime** — first `spark-consumer` start requires internet access; subsequent restarts re-download if the container is recreated.
6. **Two write paths can race** — running `pipeline.py` and `spark_consumer.py` simultaneously is safe (MERGE is idempotent) but `edit_count` may double-count.
